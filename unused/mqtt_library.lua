...
MQTT.Utility = require("mqtt_utility")

MQTT.VERSION = 0x03

MQTT.ERROR_TERMINATE = false
-- Message handler errors terminate process ?

MQTT.DEFAULT_BROKER_HOSTNAME = "home.scattym.com"

MQTT.client = {}
MQTT.client.__index = MQTT.client

MQTT.client.DEFAULT_PORT             = 65534
...
function MQTT.client:connect(                                                                         -- Public API
    ...
    local result, socket_fd = tcp.connect_host(7, self.hostname, self.port)
    if result and socket_fd == -1 then
        logger(30, "unable to connect to host")
        self.socket_client = nil
        self.connected = true
        return("MQTT.client:connect(): Couldn't open MQTT broker connection")
    end
    self.socket_client = socket_fd
    self.connected = true
...
function MQTT.client:disconnect()                                                                 -- Public API
    ...
    if (self.connected) then
        ...
        socket.close(self.socket_client)
        logger(30, "connection closed\r\n")
        tcp.close_network(7)
        --self.socket_client:close()
        self.connected = false
    else
        logger(30, "MQTT.client:disconnect(): Already disconnected")
    end
end
...
function MQTT.client:handler()                                                                        -- Public API
    ...
    if (self.connected == false) then
        logger(30, "MQTT.client:handler(): Not connected")
    end
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    local ready, closed = tcp.wait_read_event(self.socket_client, 100000)
    -- local ready = MQTT.Utility.socket_ready(self.socket_client)

    if (ready) then
        local error_message, err_code
        local buffer = ""
        err_code, buffer = socket.recv(self.socket_client, 1000)
        logger(30, "err_code: ", err_code, " buffer ", buffer)
        if (err_code and (err_code == SOCK_RST_SOCK_FAILED)) then
            logger(30, "Client closed connection")
...
-- TODO: Complete implementation of fixed header byte 1
    ...
    local err_code, bytes = socket.send(self.socket_client, message)
    logger(30, "send response. err_code: ", err_code, " bytes: ", bytes)
    if (err_code and (err_code == SOCK_RST_OK)) then
        status = 1
    else
        status = nil
        error_message = "Client went away"
    end
...
--             This would nicely collapse the if .. then .. elseif .. end.
    ...
    elseif (message_type == MQTT.message.TYPE_PUBACK) then
        logger(30, "MQTT.client:parse_message(): PUBACK -- UNIMPLEMENTED --")        -- TODO
...
function MQTT.client:parse_message_conack(                                            -- Internal API
    ...
    if (remaining_length ~= 2) then
        logger(30, me .. ": Invalid remaining length")
    end
    ...
    if (return_code ~= 0) then
        ...
        logger(30, me .. ": Connect failed: " .. error_message)
    end
end
...
function MQTT.client:parse_message_publish(                                         -- Internal API
    ...
    if (self.callback ~= nil) then
        if (remaining_length < 3) then
            logger(30, me .. ": Invalid remaining length: " .. remaining_length)
        end
...
function MQTT.client:parse_message_suback(                                            -- Internal API
    ...
    if (outstanding == nil) then
        logger(30, me .. ": No outstanding message: " .. message_id)
    end

    self.outstanding[message_id] = nil

    if (outstanding[1] ~= "subscribe") then
        logger(30, me .. ": Outstanding message wasn't SUBSCRIBE")
    end

    local topic_count = table.getn(outstanding[2])

    if (topic_count ~= remaining_length - 2) then
        logger(30, me .. ": Didn't received expected number of topics: " .. topic_count)
    end
end
...
function MQTT.client:parse_message_unsuback(                                        -- Internal API
    ...
    if (outstanding[1] ~= "unsubscribe") then
        logger(30, me .. ": Outstanding message wasn't UNSUBSCRIBE")
    end
end
...
function MQTT.client:ping_response()                                                        -- Internal API
    ...
    if (self.connected == false) then
        logger(30, "MQTT.client:ping_response(): Not connected")
    end
...
function MQTT.client:publish(                                                                         -- Public API
    ...
    if (self.connected == false) then
        logger(30, "MQTT.client:publish(): Not connected")
    end
...
function MQTT.client:subscribe(                                                                     -- Public API
    ...
    if (self.connected == false) then
        logger(30, "MQTT.client:subscribe(): Not connected")
    end
...
function MQTT.client:unsubscribe(                                                                 -- Public API
    ...
    if (self.connected == false) then
        logger(30, "MQTT.client:unsubscribe(): Not connected")
    end
